...
{
    SimpleWall()
        : IWall(TypeOfObject::Simple) {}
};

struct SimpleRoom : IRoom
{
    SimpleRoom()
        : IRoom(TypeOfObject::Simple) {}
};

TEST(AbstractFactory, InitSimpleMaze)
{
    note() << "Simple game: we have rooms and walls in our maze."
...
{
    note() << "Lets use abstract factory";
    ...
    note() << "Looks much better. I guess, we can just move it into separate function!";
}
...
{
    note() << "Lets extract method and put in factory!";
    exec(CreateWithFactory(std::make_unique<MagicFactory>()););
    note() << "And another one!";
